...
public class BlackFigureS : MonoBehaviour, IS.IBlackAndHWhite
{
    //=========================================================== –едактор
    ...
    public float health; //обновл€етс€ в специальном классе фигуры 
    ...
    void AnimationMove(float tarDur)
    {
        if (playMoving)
        {
            ...
            {
                currentCell--;
                board.cells[currentCell + 1].OutFigure(); //прошла€ €чейка открываетс€
                board.cells[currentCell].GetFigure(gameObject); //текуща€ €чейка закрываетс€
